...
def getRep(imgPath, multiple=False):
    ...
    for bb in bbs:
        ...
        if alignedFace is None:
            ...
            os.system("rm -r "+os.path.join(imgPath))
        if args.verbose:
...
def infer(args, multiple=False):
	...
	img_names.sort()
        for img_in in img_names:
            reps = getRep(img+"/"+img_in, multiple)
            if len(reps) > 1:
            ...
            for r in reps:
                ...
                if confidence >= 0.8:
                    print("Alert")
                    os.system("mv "+os.path.join(img+"/"+img_in) + " prediction_time/"+os.path.join(str(os.path.splitext(img_in)[0]) + str(datetime.datetime.now().time())+str(os.path.splitext(img_in)[1])))
                else:
                    os.system("rm -r "+os.path.join(img+"/"+img_in))
